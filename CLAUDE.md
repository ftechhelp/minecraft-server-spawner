...
- `utils/spawner.py` (`Spawner`) — registry of all spawns. Generates each spawn's `docker-compose.yml`, loads existing spawns from disk at startup (`loadSpawns()`), finds free ports, and handles archived-backup restore. Startup only loads spawn metadata; it does not start servers, so egg capacity remains enforced through the normal start routes.
